#include <iostream>
#include <cstring>
#include <sstream>
#include <iomanip>
#include <vector>

using namespace std;

//t should be 16 bytes long
void printHex(const unsigned char* t) {
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            std::cout << std::hex << int(t[i + j * 4]) << " ";
        }
        std::cout << std::endl;
    }
}

class AES {
    public:
    
    AES() = delete;
    
    AES(const unsigned char* key) {
        if (std::strlen(reinterpret_cast<const char*>(key)) != 16) {
            std::cout << "Invalid key length" << std::endl;
        }
        state = static_cast<unsigned char*>(malloc(16));
        roundKeys = generateRoundKeys(key);
    }
    
    ~AES() {
        free(state);
        for (unsigned char* k : roundKeys) {
            free(k);
        }
    }
    
    void encrypt(const unsigned char* plainText, unsigned char* cipherText) {
        std::memcpy(state, plainText, 16);
        
        //Round 0
        addRoundKey(roundKeys[0]);
        
        //Rounds 1 up to 9 
        for (int i = 1; i <= 9; ++i) {
            subBytes();
            shiftRows();
            mixColumns();
            addRoundKey(roundKeys[i]);
        }
        
        //Final round
        subBytes();
        shiftRows();
        addRoundKey(roundKeys[10]);
        
        std::memcpy(cipherText, state, 16);
    }
    
    void encryptWhiteBox(const unsigned char* plainText, unsigned char* cipherText) {
        std::vector<unsigned char*> shiftedKeys = generateShiftedRoundKeys();
        unsigned char* tboxes = static_cast<unsigned char*>(malloc(256 * 16 * 10));
        
        unsigned char* state = static_cast<unsigned char*>(malloc(16));
        std::memcpy(state, plainText, 16);
        
        
        for (int i = 0; i < 10; ++i) {
            for (int j = 0; j < 16; ++j) {
                for (int k = 0; k < 256; ++k) {
                    tboxes[i * 16 * 256 + j * 256 + k] = sBox[k ^ shiftedKeys[i][j]];
                }
            }
        }
        
        stringstream newState;
        for (int i = 0; i < 9; ++i) {
            newState <<  tboxes[i * 16 * 256 + 0x0 * 256 + state[0x0]];
            newState <<  tboxes[i * 16 * 256 + 0x1 * 256 + state[0x5]];
            newState <<  tboxes[i * 16 * 256 + 0x2 * 256 + state[0xa]];
            newState <<  tboxes[i * 16 * 256 + 0x3 * 256 + state[0xf]];
            newState <<  tboxes[i * 16 * 256 + 0x4 * 256 + state[0x4]];
            newState <<  tboxes[i * 16 * 256 + 0x5 * 256 + state[0x9]];
            newState <<  tboxes[i * 16 * 256 + 0x6 * 256 + state[0xe]];
            newState <<  tboxes[i * 16 * 256 + 0x7 * 256 + state[0x3]];
            newState <<  tboxes[i * 16 * 256 + 0x8 * 256 + state[0x8]];
            newState <<  tboxes[i * 16 * 256 + 0x9 * 256 + state[0xd]];
            newState <<  tboxes[i * 16 * 256 + 0xa * 256 + state[0x2]];
            newState <<  tboxes[i * 16 * 256 + 0xb * 256 + state[0x7]];
            newState <<  tboxes[i * 16 * 256 + 0xc * 256 + state[0xc]];
            newState <<  tboxes[i * 16 * 256 + 0xd * 256 + state[0x1]];
            newState <<  tboxes[i * 16 * 256 + 0xe * 256 + state[0x6]];
            newState <<  tboxes[i * 16 * 256 + 0xf * 256 + state[0xb]];
            
            std::memcpy(state, newState.str().c_str(), 16);
            newState.str("");
            mixColumns(state);
        }
        
        newState.str("");
        newState <<  tboxes[9 * 16 * 256 + 0x0 * 256 + state[0x0]];
        newState <<  tboxes[9 * 16 * 256 + 0x1 * 256 + state[0x5]];
        newState <<  tboxes[9 * 16 * 256 + 0x2 * 256 + state[0xa]];
        newState <<  tboxes[9 * 16 * 256 + 0x3 * 256 + state[0xf]];
        newState <<  tboxes[9 * 16 * 256 + 0x4 * 256 + state[0x4]];
        newState <<  tboxes[9 * 16 * 256 + 0x5 * 256 + state[0x9]];
        newState <<  tboxes[9 * 16 * 256 + 0x6 * 256 + state[0xe]];
        newState <<  tboxes[9 * 16 * 256 + 0x7 * 256 + state[0x3]];
        newState <<  tboxes[9 * 16 * 256 + 0x8 * 256 + state[0x8]];
        newState <<  tboxes[9 * 16 * 256 + 0x9 * 256 + state[0xd]];
        newState <<  tboxes[9 * 16 * 256 + 0xa * 256 + state[0x2]];
        newState <<  tboxes[9 * 16 * 256 + 0xb * 256 + state[0x7]];
        newState <<  tboxes[9 * 16 * 256 + 0xc * 256 + state[0xc]];
        newState <<  tboxes[9 * 16 * 256 + 0xd * 256 + state[0x1]];
        newState <<  tboxes[9 * 16 * 256 + 0xe * 256 + state[0x6]];
        newState <<  tboxes[9 * 16 * 256 + 0xf * 256 + state[0xb]];
        
        std::memcpy(state, newState.str().c_str(), 16);
        addRoundKey(shiftedKeys[shiftedKeys.size() - 1], state);
        std::memcpy(cipherText, state, 16);
        
        free(state);
        free(tboxes);
        
        for (auto k: shiftedKeys) {
            free(k);
        }
    }
    
    
    void decrypt(const unsigned char* cipherText, unsigned char* plainText) {
        std::memcpy(state, cipherText, 16);
        
        //Round 0
        const int nRounds = roundKeys.size();
        addRoundKey(roundKeys[nRounds - 1]);
        
        //Rounds 1 up to 9 
        for (int i = 9; i >= 1; --i) {
            invSubBytes();
            invShiftRows();
            invMixColumns();
            unsigned char* tmp = static_cast<unsigned char*>(malloc(16));
            std::memcpy(tmp, roundKeys[i], 16);
            invMixColumns(tmp);
            addRoundKey(tmp);
            free(tmp);
        }
        
        //Final round
        invSubBytes();
        invShiftRows();
        addRoundKey(roundKeys[0]);
        
        std::memcpy(plainText, state, 16);
    }
    
    private:
    unsigned char* state;
    std::vector<unsigned char*> roundKeys;
    
    const unsigned char sBox[256] = {
        
    };
    
    const unsigned char invSbox[256] = {
        
    };
    
    const unsigned char rCon[40] = {
        0x01, 0x00, 0x00, 0x00,
        0x02, 0x00, 0x00, 0x00,
        0x04, 0x00, 0x00, 0x00,
        0x08, 0x00, 0x00, 0x00,
        0x10, 0x00, 0x00, 0x00,
        0x20, 0x00, 0x00, 0x00,
        0x40, 0x00, 0x00, 0x00,
        0x80, 0x00, 0x00, 0x00,
        0x1b, 0x00, 0x00, 0x00,
        0x36, 0x00, 0x00, 0x00
    };
    
    void subBytes() {
        for (int i = 0; i < 16; ++i) {
            state[i] = sBox[(state[i] / 16) * 16 + (state[i] % 16)];
        }
    }
    
    void shiftRows() {
        std::stringstream result;
        for (int j = 0; j < 4; ++j) {
            for (int i = 0; i < 4; ++i) {
                result << state[i + 4 * ((i + j) % 4)];
            }
        }
        std::memcpy(state, result.str().c_str(), 16);
    }
    
    std::vector<unsigned char*> generateShiftedRoundKeys() {
        std::vector<unsigned char*> shiftedKeys;
        
        for (int i = 0; i < roundKeys.size() - 1; ++i) {
            shiftedKeys.push_back(shiftRowsKey(roundKeys[i]));
        }
        unsigned char* res = static_cast<unsigned char*>(std::malloc(16));
        std::memcpy(res, roundKeys[roundKeys.size() - 1], 16);
        shiftedKeys.push_back(res);
        
        return shiftedKeys;
    }
    
    unsigned char* shiftRowsKey(unsigned char* k) {
        unsigned char* res = static_cast<unsigned char*>(std::malloc(16));
        std::stringstream result;
        for (int j = 0; j < 4; ++j) {
            for (int i = 0; i < 4; ++i) {
                result << k[i + 4 * ((i + j) % 4)];
            }
        }
        std::memcpy(res, result.str().c_str(), 16);
        
        return res;
    }
    
    
    unsigned char mulBy2(unsigned char x) {
        unsigned char result;
        unsigned char rightShift = (x >> 7) & 1;
        result = (x << 1);
        
        return result ^ (rightShift * 0x1b);
    }
    
    void mixColumns(unsigned char* st = nullptr) {
        if (!st) {
            for (int i = 0; i < 4; ++i) {
                unsigned char tmp[4];
                unsigned char multipliedBy2[4]; //Keeps element in column multipled by 2
                std::memcpy(tmp, state + i * 4, 4); //The ith column is copied to tmp
                
                //Multiplies each column with {02} 
                for (int j = 0; j < 4; ++j) {
                    multipliedBy2[j] = mulBy2(state[i * 4 + j]); 
                }
                
                //Multiplication by {03} is the same as first multiplied by {02} and XORed with {01} 
                state[i * 4] = multipliedBy2[0] ^ tmp[3] ^ tmp[2] ^ multipliedBy2[1] ^ tmp[1];     
                state[i * 4 + 1] = multipliedBy2[1] ^ tmp[0] ^ tmp[3] ^ multipliedBy2[2] ^ tmp[2]; 
                state[i * 4 + 2] = multipliedBy2[2] ^ tmp[1] ^ tmp[0] ^ multipliedBy2[3] ^ tmp[3]; 
                state[i * 4 + 3] = multipliedBy2[3] ^ tmp[2] ^ tmp[1] ^ multipliedBy2[0] ^ tmp[0]; 
            }
        }
        
        else {
            for (int i = 0; i < 4; ++i) {
                unsigned char tmp[4];
                unsigned char multipliedBy2[4]; //Keeps element in column multipled by 2
                std::memcpy(tmp, st + i * 4, 4); //The ith column is copied to tmp
                
                //Multiplies each column with {02} 
                for (int j = 0; j < 4; ++j) {
                    multipliedBy2[j] = mulBy2(st[i * 4 + j]); 
                }
                
                //Multiplication by {03} is the same as first multiplied by {02} and XORed with {01} 
                st[i * 4] = multipliedBy2[0] ^ tmp[3] ^ tmp[2] ^ multipliedBy2[1] ^ tmp[1];     
                st[i * 4 + 1] = multipliedBy2[1] ^ tmp[0] ^ tmp[3] ^ multipliedBy2[2] ^ tmp[2]; 
                st[i * 4 + 2] = multipliedBy2[2] ^ tmp[1] ^ tmp[0] ^ multipliedBy2[3] ^ tmp[3]; 
                st[i * 4 + 3] = multipliedBy2[3] ^ tmp[2] ^ tmp[1] ^ multipliedBy2[0] ^ tmp[0]; 
            }
        }
    }
    
    std::string rotWord(unsigned char* word) {
        std::stringstream ss;
        for (int i = 0; i < 4; ++i) {
            ss << word[(i + 1) % 4];
        }
        
        return ss.str();
    }
    
    void invSubBytes() {
        for (int i = 0; i < 16; ++i) {
            state[i] = invSbox[(state[i] / 16) * 16 + (state[i] % 16)];
        }
    }
    
    void invShiftRows() {
        std::stringstream result;
        for (int j = 0; j < 4; ++j) {
            for (int i = 0; i < 4; ++i) {
                const int col = (i - j <= 0 ? i - j + 4 : i - j);
                result << state[i + 4 * (4 - col)];
            }
        }
        std::memcpy(state, result.str().c_str(), 16);
    }
    
    unsigned char mulBy9(unsigned char x)
    {
        return mulBy2(mulBy2(mulBy2(x))) ^ x;
    }
    
    unsigned char mulBy11(unsigned char x)
    {
        return mulBy2(mulBy2(mulBy2(x)) ^ x) ^ x;
    }
    
    unsigned char mulBy13(unsigned char x)
    {
        return mulBy2(mulBy2(mulBy2(x) ^ x)) ^ x;
    }
    
    unsigned char mulBy14(unsigned char x)
    {
        return mulBy2(mulBy2(mulBy2(x) ^ x) ^ x);
    }
    
    void invMixColumns(unsigned char* key = nullptr) {
        unsigned char* target = !key ? state : key;
        for (int i = 0; i < 4; ++i) {
            unsigned char tmp[4];
            std::memcpy(tmp, target + i * 4, 4); //The ith column is copied to tmp
            
            target[i * 4] = mulBy14(tmp[0]) ^ mulBy11(tmp[1]) ^ mulBy13(tmp[2]) ^ mulBy9(tmp[3]);     
            target[i * 4 + 1] = mulBy9(tmp[0]) ^ mulBy14(tmp[1]) ^ mulBy11(tmp[2]) ^ mulBy13(tmp[3]); 
            target[i * 4 + 2] = mulBy13(tmp[0]) ^ mulBy9(tmp[1]) ^ mulBy14(tmp[2]) ^ mulBy11(tmp[3]); 
            target[i * 4 + 3] = mulBy11(tmp[0]) ^ mulBy13(tmp[1]) ^ mulBy9(tmp[2]) ^ mulBy14(tmp[3]); 
        }
    }
    
    std::vector<unsigned char*> generateRoundKeys(const unsigned char* key) {
        std::vector<unsigned char*> result;
        //For the Round 0
        unsigned char* roundKey = static_cast<unsigned char*>(malloc(16));
        std::memcpy(roundKey, key, 16);
        result.push_back(roundKey);
        
        //For remaining 10 Rounds
        for (int i = 0; i < 10; ++i) {
            roundKey = static_cast<unsigned char*>(malloc(16));
            const unsigned char* rWord = reinterpret_cast<const unsigned char*>(rotWord(result[i] + 12).c_str());
            for (int k = 0; k < 4; ++k) {
                roundKey[k] = result[i][k] ^ sBox[(rWord[k] / 16) * 16 + (rWord[k] % 16)] ^ rCon[i * 4 + k];
            }
            for (int j = 1; j < 4; ++j) {
                for (int h = 0; h < 4; ++h) {
                    roundKey[4 * j + h] = result[i][4 * j + h] ^ roundKey[4 * (j - 1) + h];
                }
            }
            result.push_back(roundKey);
        }
        return result;
    }
    
    void addRoundKey(const unsigned char* roundKey, unsigned char* st= nullptr) {
        for (int i = 0; i < 16; ++i) {
            st ? st[i] ^= roundKey[i] : state[i] ^= roundKey[i];
        }
    }
};

int main()
{
    AES a(reinterpret_cast<const unsigned char*>("Hi,I am your key"));
    unsigned char* cipherText = static_cast<unsigned char*>(std::malloc(16));
    const unsigned char* plainText = reinterpret_cast<const unsigned char*>("Yeah you know me");
    
    std::cout << "Plaintext:" << std::endl;
    printHex(plainText);
    std::cout << std::endl;
    
    a.encryptWhiteBox(plainText, cipherText);
    std::cout << "Cyphertext:" << std::endl;
    printHex(cipherText);
    std::cout << std::endl;
    
    unsigned char* p = static_cast<unsigned char*>(std::malloc(16));
    a.decrypt(cipherText, p);
    std::cout << "Plaintext:" << std::endl;
    printHex(p);

    free(cipherText);
    free(p);
    
    return 0;
}